...
struct AlfredView: View {
    ...
    var body: some View {
                ...
                AlfredAvatarView(state: vm.state, isPressing: isPressing)
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { _ in
                                if !isPressing { isPressing = true; vm.startListening() }
                            }
                            .onEnded { _ in
                                if isPressing { isPressing = false; vm.stopListening() }
                            }
                    )
        ...
        .sheet(item: $vm.card)       { card in CardView(card: card) }
        .sheet(isPresented: $showOffice)     { OfficeDashboardView() }
        .sheet(isPresented: $showFamily)     { FamilyView() }
        .sheet(isPresented: $showTranslate)  { TranslateView() }
    ...
    var bottomBar: some View {
        HStack(spacing: 0) {
            bottomBtn(icon: "person.3",     label: "家人",   action: { showFamily = true })
            bottomBtn(icon: "clock.badge.checkmark", label: "出勤", action: { showAttendance = true })
            bottomBtn(icon: "globe",        label: "翻譯",   action: { showTranslate = true })
            bottomBtn(icon: "building.2",   label: "辦公室", action: { showOffice = true })
        }
        .padding(.horizontal, 12)
        .padding(.vertical, 10)
        .background(Color(hex: "#c9a84c08"))
        .overlay(RoundedRectangle(cornerRadius: 20).stroke(Color(hex: "#c9a84c20"), lineWidth: 1))
        .cornerRadius(20)
        .padding(.horizontal, 32)
    }

    func bottomBtn(icon: String, label: String, action: @escaping () -> Void) -> some View {
        ...
        ZStack {
            ForEach(0..<3) { i in
                Circle()
                    ...
                    .animation(.easeInOut(duration: 2.4).repeatForever().delay(Double(i) * 0.5),
                               value: pulse)
            }
            Circle()
                .fill(Color(hex: "#c9a84c").opacity(0.08))
                .overlay(Circle().stroke(Color(hex: "#c9a84c").opacity(0.4), lineWidth: 1.5))
        ...
        NavigationStack {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button("關閉") { dismiss() }.foregroundColor(Color(hex: "#c9a84c"))
                }
            }
        }
    }
...
extension Text {
    func letterSpacing(_ spacing: CGFloat) -> some View { self.kerning(spacing) }
}
